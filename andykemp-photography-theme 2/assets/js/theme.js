...
(function($) {
    ...
    function initHeroTextParallax() {
        ...
        function updateFloatEffect() {
            ...
            // Start floating when the section would normally start to disappear
            const triggerPoint = descriptionOffset - windowHeight + 100;
            // End floating after scrolling past hero
            const endPoint = heroHeight + 200;
            
            if (scrolled > triggerPoint && scrolled < endPoint) {
                // Floating over hero
                heroDescSection.classList.add('floating');
                heroDescSection.classList.remove('fading-out');
            } else if (scrolled >= endPoint) {
